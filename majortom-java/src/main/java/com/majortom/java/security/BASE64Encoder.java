package com.majortom.java.security;
...
public class BASE64Encoder {
    ...
    private void decodeAtom(PushbackInputStream paramPushbackInputStream, OutputStream paramOutputStream, int paramInt)
        ...
        i = readFully(paramPushbackInputStream, decodeBuffer, 1, paramInt - 1);
        if (i == -1) {
            throw new RuntimeException();
        }

        if (paramInt > 3 && decodeBuffer[3] == 61) {
            paramInt = 3;
        }
        if (paramInt > 2 && decodeBuffer[2] == 61) {
            paramInt = 2;
        }
        switch (paramInt) {
        case 4:
            n = pem_convert_array[decodeBuffer[3] & 0xFF];
        case 3:
            m = pem_convert_array[decodeBuffer[2] & 0xFF];
        case 2:
            k = pem_convert_array[decodeBuffer[1] & 0xFF];
            j = pem_convert_array[decodeBuffer[0] & 0xFF];
        }
